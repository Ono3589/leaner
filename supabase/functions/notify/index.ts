...
/* ------------------------------------------------------------
   ...
   Die verbreitete Bibliothek "web-push" ist für Node geschrieben und
   braucht crypto.createECDH — das fehlt in Denos Nachbau. Deshalb hier
   selbst umgesetzt, nach RFC 8291 (Verschlüsselung) und RFC 8292 (VAPID).
   Alles über crypto.subtle, ohne fremde Abhängigkeit.

   Bewusst in dieser einen Datei: So lässt sich die Funktion im
   Supabase-Editor mit einem einzigen Einfügen veröffentlichen.
------------------------------------------------------------ */
...
async function encryptPayload(payload: string, p256dhB64: string, authB64: string) {
  ...
  const ciphertext = new Uint8Array(await crypto.subtle.encrypt(
    { name: 'AES-GCM', iv: nonce as BufferSource, tagLength: 128 }, aesKey, plaintext as BufferSource
  ));
